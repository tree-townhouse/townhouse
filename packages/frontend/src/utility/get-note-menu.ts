...
export function getNoteMenu(props: {
	...
	if ($i) {
		...
		if (appearNote.channel && appearNote.userId !== $i.id && !$i.isRestricted && (appearNote.channel.userId === $i.id || appearNote.channel.isViewerChannelManager || $i.isModerator || $i.isAdmin)) {
			menuItems.push({ type: 'divider' });
			menuItems.push({
				icon: appearNote.isBlinded ? 'ti ti-eye' : 'ti ti-eye-off',
				text: appearNote.isBlinded ? i18n.ts.unblind : i18n.ts.blind,
